...
const userSchema = z.object({
  ...
  senha: z
    .string()
    .min(6, { message: "Informe a senha com pelo menos 6 caracteres" }),
});
...
export default function Login() {
  ...
  async function loginAction(previousState, formData) {
    ...
    try {
      ...
      return {
        errors: {
          form: error.response?.data?.message || "Erro ao tentar fazer login",
        },
        success: false,
      };
    }
        ...
        <div className="bg-white/60 backdrop-blur-sm border border-tinta/10 rounded-2xl p-7 shadow-sm">
          ...
          <form action={formAction} className="space-y-4">
            ...
            <div className="mt-6 text-center">
              <p className="text-xs text-tinta/70 font-sans">
                Não tem uma conta?{" "}
                <NavLink
                  to="/register" 
